...
Для запуска требуется указать в аргументах запуска абсолютный путь к файлу с входными данными. Значения образуют строку, в которой они разделены символом ';'. Строки значений разделены между собой переносом строки.
<h2>Информация по сборке</h2>
Сборка осуществляется через Gradle с использованием <a href="https://plugins.gradle.org/plugin/com.github.johnrengelman.shadow">johnrengelman.shadow</a><br>
Для билда jar-файла используется ./gradle shadowJar
<h1>Текст задания</h1>
С помощью Java:
